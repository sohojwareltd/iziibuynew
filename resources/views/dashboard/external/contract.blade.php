<x-dashboard.external>
    ...
    <div class="row mt-5">
        ...
        @else
            ...
            <div class="row g-3">
                @foreach ($paymentMethodAccess->gateway_contract_signed ? json_decode($paymentMethodAccess->gateway_contract_signed) : [] as $gateway => $status)
                    <div class="col-12">
                        @include("dashboard.external.contract.$gateway", ['status' => $status])
                    </div>
                @endforeach
            </div>
