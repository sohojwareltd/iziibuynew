<x-dashboard.shop>
    ...
    <div class="container mb-3">
        <div>
            <a href="{{ route('shop.download.invoice', $charge) }}" class="btn btn-primary"><i class="fa fa-download"></i>
                {{ __('words.download_invoice') }}</a>
        </div>
    </div>
    @if (count($datas))
        <div class="container">
            ...
            <div class="row ">
                <div class="col-md-3">
                    <div class="card auth-card">
                        ...
                        <h6 class="auth-price">{{ Iziibuy::round_num($transiction->amount / 100) }}
                            {{ $transiction->currency }} <a href=""
            ...
            <div class="container">
                ...
                @if (isset($transiction->settlement))
                    ...
                    <div class="card border-radius">
                        <table class="table table-hover">
                            ...
                            <tr>
                                <td>Fees</td>
                                <td>{{ Iziibuy::round_num($transiction->settlement->fees / 100) }}
                            ...
                            <tr>
                                ...
                                @foreach ($transiction->settlement->fee_details as $auth)
                                    ...
                                    <td class="border-bottom-1 text-muted">
                                        {{ Iziibuy::round_num($auth->amount / 100) }}
                                        {{ $transiction->settlement->currency }}</td>
                                @endforeach
                ...
                <div class="d-flex ">
                    ...
                    @foreach ($datas as $key => $data)
                        <a href="{{ route('shop.charge.invoice', [$charge, 'mode' => $key]) }}" class=" gap-4">
                            <div class='card auth-card-2 @if ($transiction->type == $data->type) active-footer @endif'>
                                ...
                                <h6 class="auth-price text-muted mt-1">{{ Iziibuy::round_num($data->amount / 100) }}
                                    {{ $data->currency }}
    ...
    @else
        <div class="container text-center">
            <div class="alert alert-warning" role="alert">
                {{ __('words.test_card_warning') }}
                <br>
                <a href="{{ route('shop.download.invoice', $charge) }}" >
                    {{ __('words.download_invoice') }}</a>
            </div>
