<x-shop-front-end>
    ...
    <div class="container my-5">
        <div class="row">
            @foreach ($trainers as $trainer)
                    ...
                    <div class="card ">
                        <div class="card-body">
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <img class="rounded-circle mb-2 " style="width: 60px;height:60px"
                                    src="{{ Iziibuy::image($trainer->avatar) }}" height="40" alt="User">
                                <div>
                                    <h5>{{ $trainer->full_name }}
                                        @if ($trainer->level)
                                            <small
                                                class="text-secondary">({{ ucwords($trainer->level()->title) }})</small>
                                        @endif
                                    </h5>
                                    <small class="text-secondary">{{ $trainer->email }}</small>
                                    <div class="d-flex justify-content-end mt-2">
                                        <i class="fa fa-clock btn p-1 mx-2" data-toggle="modal"
                                            data-target="#avaliable{{ $trainer->id }}"></i>
                                        <i class="fa fa-calendar btn p-1" data-toggle="modal" data-target="#schedule"
                                            data-user="{{ $trainer->id }}"></i>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="modal fade" id="avaliable{{ $trainer->id }}" tabindex="-1" role="dialog"
    ...
    <form id="booking-form" action=""></form>
            ...
            const confirm = (data) => {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        let url =
                            `${baseUrl}/shop/${shop}/confirm-free-appointment-booking?date=${data.dataset.date}&time=${data.dataset.slot}&trainer=${document.user.id}`;
