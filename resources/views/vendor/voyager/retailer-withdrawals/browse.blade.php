                                    ...
                                    @foreach ($dataTypeContent as $data)
                                        @if ($data->getAttribute('status') == 0)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($data->user->hasBankAccount())
                                                        <a href="{{ route('admin.retailer.withdraw.approve', $data) }}"
                                                            class="btn btn-sm btn-success">Approve</a>
                                                    @else
                                                        <a target="_blank" class="btn btn-danger"
                                                            href="{{ \App\Filament\Resources\RetailerMetas\RetailerMetaResource::getUrl('edit', ['record' => $data->user->retailer->id], panel: 'admin') }}">Update
                                                            Bank account</a>
                                                    @endif
                                                    <a href="{{ route('admin.retailer.withdraw.cancel', $data) }}"
