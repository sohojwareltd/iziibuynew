    ...
    <div class="page-content edit-add container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                @if ($edit)
                    @if ($dataTypeContent->status == 0 && $dataTypeContent->isClientTransfered == false)
                        @php
                            $retailers = App\Models\User::where('role_id', 5)
                                ...
                                ->map(function ($retailer) {
                                    return [
                                        'id' => $retailer->id,
                                        'name' => $retailer->full_name . ' (' . $retailer->email . ')',
                                    ];
                                });
                        @endphp
                        <form action="{{ route('admin.retailer.transferClients', $dataTypeContent) }}" method="post">
                            @csrf
                            <div style="border: 1px #504f4f solid; padding:10px">
                                <div class="form-group">
                                    <label for="">Transfer clients to</label>
                                    <select name="transfer_to" style="border: 1px #504f4f solid;" class="form-control"
                                        id="">
                                ...
                                <button class="btn btn-primary">Tranfer</button>
                            </div>
                        </form>

                    @endif
                    @if ($dataTypeContent->isClientTransfered == true && $dataTypeContent->clientTransferedTo)
                        <h4 class="text-center">All the client is transfered to <strong class="text-primary">

                                {{ App\Models\User::find($dataTypeContent->clientTransferedTo)->full_name }}</h4>
                        </strong>
                    @endif
                    @php
                        $shops = App\Models\Shop::where('retailer_id', $dataTypeContent->user_id)->get();
                    @endphp
                    @if ($shops->count())
                        <h5>
                            Shops
